...
module.exports = class controllerUsuario {
  static async cadastraUsua(req, res) {
    const {data, horario, estado, IdSala, IdUsuario} = req.body;

    if (!data || !horario || !estado || !IdSala || !IdUsuario) {
      return res
        .status(400)
    ...
    const query = `INSERT INTO usuario(data, horario, estado, IdSala, IdUsuario) VALUES ('${data}', '${horario}', '${estado}', '${IdSala}', '${IdUsuario}');`;
  ...
  static async loginUsua(req, res) {
    ...
    const query = `SELECT * FROM usuario WHERE data = ? AND estado = ?`;
    const values = [data, horario, estado, IdSala, IdUsuario];
